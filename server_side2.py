...
def client_mungling(connection, address):
    ...
    while connected:
        msg_length = connection.recv(HEADER).decode(
            FORMAT)  # receiver the size (HEADER) the message must be & decodes it from binary to utf-8
        if msg_length:  # Checks if there is an actual message to send (if somebody has connected)
            ...
            message = connection.recv(msg_length).decode(
                FORMAT)  # actual message received with the proper length and decoded to uft-8
            if message == DISCONNECTED:  # disconnects user (not to cause problems later)
                connected = False
            DICT_IP[message.split('>')[0]] = address
            print(address)
            print(DICT_IP)
            print(f"User [{address}] {message.split('>')[0]}  said : {message.split('>')[1]}")
            connection.send(f"{message.split('>')[1]} received".encode(FORMAT))
    connection.close()
...
def start():
    ...
    while True:
        ...
        thread = threading.Thread(target=client_mungling,
                                  args=(connection, address))  # Creates & sends thread to client_mungling
        thread.start()
